...
function bestFromImgTags(html, base) {
  ...
  while ((tag = imgRe.exec(html))) {
    ...
    if (isHttp(src)) candidates.push({ url: src, score });
  }
...
function collectDirectUrls(text, base) {
  ...
  for (const u of raw.match(/https?:\/\/[^\s"'<>]+/gi) || []) {
    ...
    if (isHttp(cleaned) && (IMAGE_EXT.test(cleaned) || /cloudinary|cdn|image|photo|media|upload/i.test(cleaned)) && !BAD.test(cleaned)) out.push(cleaned);
  }
  return out;
}
...
async function resolve(input) {
  ...
  ]).filter(u => isHttp(u) && !BAD.test(u));

  // Exact page candidates first. Query/hash nested links are intentionally not used here.
