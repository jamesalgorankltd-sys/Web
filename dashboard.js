...
function directFromKnownPageUrl(input){
  try{
    const u = new URL(decodeDeep(input));
    // Magnific page URLs often contain an unrelated old query image.
    // Build the real image source from the page slug first.
    const m = u.href.match(/magnific\.com\/free-ai-image\/([^?#]+?)\.htm/i);
    if(m && m[1]){
      const slug = decodeURIComponent(m[1]).replace(/[^a-z0-9_\-]/gi,'');
      if(slug) return 'https://img.magnific.com/free-photo/' + slug + '.jpg';
    }
  }catch(e){}
  return '';
}
...
async function resolveInputUrlForUpload(url){
  ...
  // Known page resolver must run before nested query/hash links.
  // Some pages keep old/unrelated image URLs inside query/hash parameters; using those first returns the wrong image.
  ...
  // Last fallback only: nested direct image URLs from pasted text.
  // This is intentionally after page/known/backend extraction to avoid wrong-page images.
...
function webpCloudinaryUrl(url){
  if(!url) return '';
  if(url.includes('/image/upload/') && !url.includes('/f_webp,')) return url.replace('/image/upload/','/image/upload/f_webp,q_auto:best/').replace(/\.(jpg|jpeg|png|gif|avif|webp)(\?.*)?$/i,'.webp');
  return url;
}
async function uploadToCloudinary(input, title='image'){
  ...
  if(input instanceof Blob){
    fd.append('file', input, 'image.webp');
  }else{
...
function clearNow(e){
  e?.preventDefault?.();
  clearPreviewUrls(); els.imageUrl.value=''; els.singleResult.classList.add('hidden'); els.finderGrid.innerHTML=''; els.finderStatus.classList.add('hidden'); els.activityLine?.classList.add('hidden'); setStatus('Ready.','Paste link, drag image, or use Featured Image Finder.'); progress(0); setButtonDone(els.clearBtn, 'Cleared ✓');
}
