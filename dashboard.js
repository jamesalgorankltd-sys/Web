...
async function resolveInputUrlForUpload(url){
  ...
  if(nestedDirect && !isWrongMagnificCandidate(nestedDirect, url)) return nestedDirect;
...
function webpCloudinaryUrl(url){
  ...
  if(url.includes('/image/upload/')){
    ...
    // q_auto:best keeps maximum WebP quality; e_improve + e_sharpen reduce soft/blurred output without changing size.
    const transformed = url.replace('/image/upload/','/image/upload/f_webp,q_auto:best,e_improve,e_sharpen:180/');
